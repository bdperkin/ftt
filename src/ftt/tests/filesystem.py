...
def test_filesystem(filepath: Path) -> TestResult:
    ...
    try:
        ...
        if filepath.is_symlink():
            target = filepath.readlink() if filepath.is_symlink() else None
        ...
        if stat.S_ISREG(mode) and mode & stat.S_IXUSR:
            ...
            # This is a basic filesystem test, more detailed checks happen in magic tests
            file_type = FileType(
                category=FileTypeCategory.EXECUTABLE,
