...
MAGIC_SIGNATURES: List[Tuple[int, bytes, str, FileTypeCategory, Optional[str]]] = [
    # Executables
    (0, b'\x7fELF', 'ELF executable', FileTypeCategory.EXECUTABLE, 'application/x-executable'),
    (0, b'MZ', 'MS-DOS executable', FileTypeCategory.EXECUTABLE, 'application/x-msdownload'),
    (0, b'\xfe\xed\xfa\xce', 'Mach-O executable (32-bit)', FileTypeCategory.EXECUTABLE, 'application/x-mach-binary'),
    (0, b'\xfe\xed\xfa\xcf', 'Mach-O executable (64-bit)', FileTypeCategory.EXECUTABLE, 'application/x-mach-binary'),
    (0, b'\xcf\xfa\xed\xfe', 'Mach-O executable (32-bit, reverse)', FileTypeCategory.EXECUTABLE, 'application/x-mach-binary'),
    (0, b'\xce\xfa\xed\xfe', 'Mach-O executable (64-bit, reverse)', FileTypeCategory.EXECUTABLE, 'application/x-mach-binary'),
    
    # Archives and compressed files (binary data)
    (0, b'PK\x03\x04', 'ZIP archive data', FileTypeCategory.DATA, 'application/zip'),
    (0, b'PK\x05\x06', 'empty ZIP archive data', FileTypeCategory.DATA, 'application/zip'),
    ...
    (0, b'\xfd7zXZ\x00', 'XZ compressed data', FileTypeCategory.DATA, 'application/x-xz'),
    (0, b'7z\xbc\xaf\x27\x1c', '7-zip archive data', FileTypeCategory.DATA, 'application/x-7z-compressed'),
    (0, b'Rar!\x1a\x07\x00', 'RAR archive data', FileTypeCategory.DATA, 'application/x-rar-compressed'),
    (0, b'Rar!\x1a\x07\x01\x00', 'RAR archive data', FileTypeCategory.DATA, 'application/x-rar-compressed'),
    
    # Tar archives (known to contain binary data)
    (257, b'ustar\x00', 'POSIX tar archive', FileTypeCategory.DATA, 'application/x-tar'),
    (257, b'ustar  \x00', 'GNU tar archive', FileTypeCategory.DATA, 'application/x-tar'),
    ...
    (0, b'\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1', 'Microsoft Office document data', FileTypeCategory.DATA, 'application/msword'),
    (0, b'PK\x03\x04\x14\x00\x06\x00', 'Microsoft Office Open XML document data', FileTypeCategory.DATA, 'application/vnd.openxmlformats'),
    
    # Core files (known to contain binary data)
    (0, b'\x7fELF', 'ELF core file data', FileTypeCategory.DATA, 'application/x-coredump'),  # Will be refined by ELF parser
    
    # Other binary formats
    (0, b'\x00\x00\x01\x00', 'Windows icon image data', FileTypeCategory.DATA, 'image/x-icon'),
    (0, b'\x00\x00\x02\x00', 'Windows cursor image data', FileTypeCategory.DATA, 'image/x-icon'),
    (0, b'\x42\x5a\x68', 'bzip2 compressed data', FileTypeCategory.DATA, 'application/x-bzip2'),
]
