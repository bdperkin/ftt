...
TEXT_CONTENT_PATTERNS = [
    ...
    (re.compile(r'^\s*SELECT\s+.*\s+FROM\s+', re.MULTILINE | re.IGNORECASE), 'SQL text'),
    (re.compile(r'^\s*INSERT\s+INTO\s+', re.MULTILINE | re.IGNORECASE), 'SQL text'),
    (re.compile(r'^\s*CREATE\s+(TABLE|DATABASE)', re.MULTILINE | re.IGNORECASE), 'SQL text'),
...
def test_language(filepath: Path) -> TestResult:
    """
    Perform language-based tests on a file to detect programming languages and text types.
    ...
    try:
        ...
        if extension in TEXT_EXTENSIONS:
            file_type = FileType(
                category=FileTypeCategory.TEXT,
                description=TEXT_EXTENSIONS[extension]
            )
            return TestResult.success_result(file_type)
